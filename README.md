...
Anime data is fetched from the [AniList GraphQL API](https://anilist.co/graphiql) (`https://graphql.anilist.co`), a free public/unofficial-use API — not an official partnership.
...
- **Third-party API**: This project uses the [AniList GraphQL API](https://anilist.co/graphiql) to fetch anime data. It is **not affiliated with, endorsed by, or supported by AniList**. Availability, rate limits, and data accuracy depend entirely on that third-party service and can change or break without notice.
...
- **Copyright**: Anime titles, posters, and metadata belong to their respective copyright holders (studios, publishers, AniList, etc.). This project only displays data fetched via the AniList API for personal reference use and claims no ownership over any anime artwork or information shown.
